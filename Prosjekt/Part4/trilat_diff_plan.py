...
def find(t,d,tidspunkt):
    p0 = d[0,:]
    p1 = d[1,:]
    s0 = d[2,:]
    x1,y1 = interpol(t,p0,tidspunkt)
    x2,y2 = interpol(t,p1,tidspunkt)
    x3,y3 = interpol(t,s0,tidspunkt)
    r1 = mission.measure_distances()[0]
    r2 = mission.measure_distances()[4]
    r3 = mission.measure_distances()[-1]
    print("r1 = {:g}, r2 = {:g}, r3 = {:g}".format(r1,r2,r3))
    return x1,y1,x2,y2,x3,y3,r1,r2,r3

#Trilaterate position
def position(t,d,tidspunkt):
    t,d,tidspunkt = t,d,tidspunkt
    x1,y1,x2,y2,x3,y3,r1,r2,r3 = find(t,d,tidspunkt)
    A = 2*x2 - 2*x1
    B = 2*y2 - 2*y1
...
planet_orbit,planet_index = integrator(Planets[0])
...
#Find position
tidspunkt = 0.4
x,y,x1,y1,x2,y2,x3,y3,r1,r2,r3 = position(t,planetorb,tidspunkt)
...
plt.plot(s3,s4,"r--")
plt.plot(s5,s6,"g--")
plt.plot(x,y,"yo", label="Rocket [{:g}, {:g}]".format(x,y))
plt.plot(x1,y1,"bo")
plt.plot(x2,y2,"go")
...
plt.legend()
plt.savefig("Trilateration_simulated_diff.jpeg")
plt.show()
