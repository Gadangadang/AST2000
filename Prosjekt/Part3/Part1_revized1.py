...
"""
"""
For oppgave 3 har vi bare endret på fuksjonen convert_to_solar_system_frame_simple.
Resten av koden er helt lik fra del 1.
"""
...
def simulate_rocket_launch(mission,
    ...
    v_esc = np.sqrt(2*G*home_planet_mass/home_planet_radii)
    ...
    fuel_mass_after_launch = rocket_mass-rocket_mass_loss_rate*t
    launch_duration = t
...
"""
Sender inn vel_at_t og position_at_t som er arrays med posisjon og hastigheten
til planeten i ønsket tidssted, t_at_launch.
Funksjonen krever også nå en vinkel, som er definert mellom x-aksen og
posisjonsvektoren til planeten. Vinkel = angle_on_planet
"""
def convert_to_solar_system_frame_simple(system,
                                         final_height_above_surface,
                                         final_upward_speed,
                                         launch_duration,angle_on_planet,
                                         t_at_launch,position_at_t,
                                         vel_at_t):
    planet_vel_x,planet_vel_y = vel_at_t
    planet_pos_x,planet_pos_y = position_at_t
    tangential_speed = mission.system.radii[0]/AU*1000*(2*pi/\
    (system.rotational_periods[0]*day/yr))#AU/yr
    """
    Bruker trigonometriske egenskaper til å ta hensynn til at den tangentielle
    hastigheten kan virke i både x- og y-retning.
    """
    tangential_speed_ar = np.array([-tangential_speed*np.sin(angle_on_planet)\
    ,tangential_speed*np.cos(angle_on_planet)])
    """
    Bruker trigonometriske egenskaper til å ta hensynn til at posisjonen til raketten
    ikke lenger er definert som y=0 og x=initiall pos + radius.
    """
    rocket_position_before_launch = [mission.system.radii[0]*1000/AU*np.cos(angle_on_planet)+\
    planet_pos_x,planet_pos_y + mission.system.radii[0]*1000/AU*np.sin(angle_on_planet)]#AU
    """
    Igjen bruker vi trigonometriske egenskaper til å ta hensynn til at
    final_upward_speed ikke lenger bare er i èn dimensjon.
    """
    rocket_velocity_after_launch = [tangential_speed_ar[0]+final_upward_speed/(AU/yr)\
    *np.cos(angle_on_planet)+planet_vel_x,\
    planet_vel_y+tangential_speed_ar[1]]+final_upward_speed/(AU/yr)*np.sin(angle_on_planet)#AU/yr


    rocket_position_after_launch = [rocket_position_before_launch[0]+\
    final_height_above_surface/AU*np.cos(angle_on_planet),rocket_velocity_after_launch[1]*launch_duration/yr]\
    + final_height_above_surface/AU*np.sin(angle_on_planet)
    time_after_launch = t_at_launch + launch_duration/yr
...
def run_engine_and_launch_simulation(mission,
    ...
    rocket_mass_loss_rate = mass_loss_rate_per_box*number_of_particles_in_box
...
if __name__ == '__main__':
    ...
    number_of_particles_in_box = 10E5
    temperature = 10E3
    box_hole_area = (1/16)*box_side_length**2
    initial_fuel_mass = 10000
    angle_on_planet = angle
    position_at_t = np.array([x_pos,y_pos])
    vel_at_t = np.array([x_vel,y_vel])
    t_at_launch = t_0
    ...
    time_after_launch              \
      = convert_to_solar_system_frame_simple(system,
                                             ...
                                             final_upward_speed,
                                             launch_duration,angle_on_planet,
                                             t_at_launch,position_at_t,vel_at_t)
    t_at_launch = 0.0
    ...
    print(rocket_position_before_launch)
