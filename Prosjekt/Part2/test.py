...
def integrator(planet,sun):
    ...
    N = 2500000
    time = 5 #time for 20 rotations for our home planet
    #print(time)
    dt = time/N
    t = dt
...
Under = -system.radii[0]*1000/AU - system.star_radius*1000/AU
x_planet = np.copy(planet_orbit)

condition1 = np.logical_and(np.greater_equal(x_planet[:,1],Under),np.less_equal(x_planet[:,0],0))
condition2 = np.logical_and(np.less_equal(x_planet[:,1],Upper),np.less_equal(x_planet[:,0],0))
...
condition3 = np.logical_and(np.greater_equal(x_planet[:,1],Under),np.less_equal(x_planet[:,0],0))
condition4 = np.logical_and(np.less_equal(x_planet[:,1],Upper),np.less_equal(x_planet[:,0],0))
