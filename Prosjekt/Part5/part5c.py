...
def send_spacecraft(interplanetary_travel,
    ...
    rock_pos =np.zeros((5000,2))#legg til 8000 punkter for 4 år med coast
    j = 0
    t = np.linspace(19.75,24.75,5000)
    ...
    print("Average orbit distance is {:g} meters or {:g} km".format(snitt_r*AU,snitt_r*AU/1000))
    ...
    plt.ylabel('AU')
    plt.axis("Equal")
    plt.savefig('Rocket_w_boost_focus_100.jpeg')
    plt.show()

    #End of simulation

    interplanetary_travel.take_picture("Pick_at_end.xml")
    interplanetary_travel.finish_video(filename='travel_video_closer.xml', number_of_frames=2000)
